...
type svcCollector10min struct {
	svcTargets []svcTarget
}

type yml_conf10min struct {
	Monitor struct {
...
func (c *conf10min) getConf() *conf10min {
...
func readConfig10min(configFile *string) *svcCollector10min {
...
func newSvcCollector10min(c *svcCollector10min) *svcCollector10min {

	fmt.Println("*****newSvcCollector10min")
...
func (c *svcCollector10min) Describe(ch chan<- *prometheus.Desc) {
...
func (c *svcCollector10min) Collect(ch chan<- prometheus.Metric) {
