...
type svcCollector15min struct {
	svcTargets []svcTarget
}

type yml_conf15min struct {
	Monitor struct {
...
func (c *conf15min) getConf() *conf15min {
...
func readConfig15min(configFile *string) *svcCollector15min {
...
func newSvcCollector15min(c *svcCollector15min) *svcCollector15min {

	fmt.Println("*****newSvcCollector15min")
...
func (c *svcCollector15min) Describe(ch chan<- *prometheus.Desc) {
...
func (c *svcCollector15min) Collect(ch chan<- prometheus.Metric) {
